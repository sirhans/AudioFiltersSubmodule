...
#define DesiredBufferLength 1024
...
float* BMSpectrum_processData(BMSpectrum* this,float* inData,int inSize,int* outSize,float* nq){
    if(inSize>0){
        ...
        *nq = this->fft_output.imagp[0];
        ...
        vDSP_zvabs(&this->fft_output, 1, magSpectrum, 1, inSize/2);
        
        //Limit the point
        float minValue = 0.0001;//-40db
        float maxValue = 1000;
        vDSP_vclip(magSpectrum, 1, &minValue, &maxValue, magSpectrum, 1, inSize/2);
        float b = 0.01;
        vDSP_vdbcon(magSpectrum, 1, &b, magSpectrum, 1, inSize/2, 1);
